/* ========================================================================
   RENOVATION PATCH: My97 DatePicker Modernization
   ======================================================================== */

// 1. Polyfill IE-specific 'attachEvent' for modern browsers
if (!window.attachEvent) {
    var _attach = function(e, h) {
        // Remove 'on' prefix (e.g., 'onclick' -> 'click')
        this.addEventListener(e.replace(/^on/, ''), h);
    };
    Window.prototype.attachEvent = _attach;
    Document.prototype.attachEvent = _attach;
    Element.prototype.attachEvent = _attach;
}

// 2. Inject the Language Object ($lang)
// This bypasses the need for 'en.js' or 'zh-cn.js' to load via iframe.
...
// 3. Robust Config Object ($dp)
// If 'parent.$dp' is blocked by Cross-Origin rules, we create a local fallback
// so the script doesn't crash immediately.
if (typeof $dp === 'undefined') {
    try {
        window.$dp = (parent && parent.$dp) ? parent.$dp : {};
    } catch (e) {
        console.warn("My97: Cross-origin blocked parent access. Using local fallback.");
        window.$dp = {};
    }
}

// 4. Safety shim for 'document.createStyleSheet' (IE specific)
if (!document.createStyleSheet) {
    document.createStyleSheet = function(cssText) {
...
/* ========================================================================
   END PATCH - Original Unpacked Code Starts Below
   ======================================================================== */
...
function My97DP() {
  ...
  for (var B = 0; B < $dp.focusArr.length; B++) {
    var _ = $dp.focusArr[B];
    _.nextCtrl =
      B == $dp.focusArr.length - 1 ? $dp.focusArr[1] : $dp.focusArr[B + 1];
...
My97DP.prototype = {
  init: function () {
    ...
    this.autoPickDate =
      $dp.autoPickDate == null
        ? $dp.has.st && $dp.has.st
          ? false
          : true
        : $dp.autoPickDate;
    $dp.autoUpdateOnChanged =
      $dp.autoUpdateOnChanged == null
        ? $dp.isShowOK && $dp.has.d
          ? false
          : true
        : $dp.autoUpdateOnChanged;
    ...
    this.minDate = this.doCustomDate(
      $dp.minDate,
      $dp.minDate != $dp.defMinDate ? $dp.realFmt : $dp.realFullFmt,
      $dp.defMinDate,
    );
    this.maxDate = this.doCustomDate(
      $dp.maxDate,
  ...
  checkValid: function ($, A, B) {
    ...
    if (_ == 0) {
      ...
      if (A == "d" && B == null)
        B = Math.abs(
          (new Date($.y, $.M - 1, $.d).getDay() - $dp.firstDayOfWeek + 7) % 7,
        );
  ...
  checkAndUpdate: function () {
    ...
    if ($dp.errDealMode >= 0 && $dp.errDealMode <= 2 && $ != null) {
      ...
      if (
        $ == "" ||
        (A.isDate(A.date) && A.isTime(A.date) && A.checkValid(A.date))
      ) {
        if ($ != "") {
  ...
  _f: function (p, c, r, e, isR) {
    ...
    for (var i = 0; i < r; i++) {
      ...
      for (var j = 0; j < c; j++) {
        ...
        $dt[p] = eval(e);
        if (
          ($dp.opposite && this.checkRange($dt, p) == 0) ||
  ...
  _fillQS: function (C, A) {
    this.initQS();
    var $ = A
        ? '>a/<rekciPetaD 79yM>knalb_=tegrat "eulb:roloc"=elyts "ten.79ym.www//:ptth"=ferh a<'
            .split("")
            .reverse()
            .join("")
        : $lang.quickStr,
    ...
    if (!C)
      _.a(
        '<div style="float:right;cursor:pointer" onclick="hide($d.qsDivSel);">\xd7</div>',
      );
    _.a("</td></tr>");
  ...
  _dealFmt: function () {
    ...
    $dp.realFullFmt = $dp.realFullFmt
      .replace(/%Date/, $dp.realDateFmt)
  ...
  initShowAndHide: function () {
    var $ = 0;
    $dp.has.y
      ? (($ = 1), show($d.yI, $d.navLeftImg, $d.navRightImg))
      : hide($d.yI, $d.navLeftImg, $d.navRightImg);
    $dp.has.M
      ? (($ = 1), show($d.MI, $d.leftImg, $d.rightImg))
  ...
  getDateStr: function (_, $) {
    ...
    if (_.indexOf("D") >= 0) {
      ...
      _ = this.getP("M", _, $);
      _ = _.replace(/\%dd/g, this.getP("D", "DD")).replace(
        /\%d/g,
        this.getP("D", "D"),
      );
  ...
  try {
    if (
      ...
      (_.nodeName.toLowerCase() == "input" ||
        _.nodeName.toLowerCase() == "textarea")
    ) {
      _["My97Mark"] = true;
      _.focus();
    }
